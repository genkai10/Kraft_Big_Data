...
def set_register_broker_record(record_data):

    # Assuming record_data is a JSON object containing broker details
    # Store the broker record in the metadata store of the node
    node.data['RegisterBrokerRecords']['records'].append(record_data)
    # Update the timestamp for RegisterBrokerRecords
    node.data['RegisterBrokerRecords']['timestamp'] = "current_timestamp"  
    
"""
# Mutex for thread safety when updating broker status
broker_status_lock = threading.Lock()

# Mutex for thread safety when updating broker epoch
...
# Function to update broker and increment the epoch
...
# Function to unregister a broker
...
# Route to update a broker and increment the epoch
